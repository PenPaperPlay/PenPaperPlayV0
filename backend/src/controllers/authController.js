...
export const registration = async (req, res, next) => {
    try {
        ...
        return res.status(201).json({ message: 'User registered successfully', user: rest, token });
    } catch (error) {
...
export const login = async (req, res) => {
    ...
    try {
        ...
        return res.status(500).json({ massage: ` Login failed becouse of ${error}` })
    }
...
export const verifyOTP = async (req, res) => {
    try {
        ...
        const resetToken = jwt.sign({ email }, process.env.RESET_PASSWORD_SECRET, { expiresIn: '5m' });
...
export const resetPassword = async (req, res) => {
    try {
        ...
        if (!resetToken) {
            return res.status(401).json({ message: 'Unauthorized: No reset token found' });
        }
